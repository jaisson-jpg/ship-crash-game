// auth-working.js - Versão que funciona baseada no debug
console.log('auth-working.js carregado');

// Aguardar o DOM estar pronto
document.addEventListener('DOMContentLoaded', () => {
  console.log('auth-working.js: DOM carregado');
  
  // Aguardar um pouco para garantir que todos os scripts foram carregados
  setTimeout(() => {
    console.log('auth-working.js: Iniciando configuração');
    ...
    console.log('auth-working.js: Elementos encontrados:', {
      loginForm: !!loginForm,
    ...
    if (!loginForm || !registerForm) {
      console.log('auth-working.js: Elementos de formulário não encontrados, saindo...');
      return;
    }

    // Verificar se Firebase está disponível
    if (typeof auth === 'undefined' || typeof db === 'undefined') {
      console.error('auth-working.js: Firebase não está disponível!');
      return;
    }

    console.log('auth-working.js: Firebase disponível, configurando eventos...');

    function showError(err) {
      console.error('auth-working.js: Erro:', err);
      if (!authMessage) {
    ...
    function showSuccess(message) {
      console.log('auth-working.js: Sucesso:', message);
      if (!authMessage) {
    ...
    if (registerBtn) {
      console.log('auth-working.js: Configurando evento de registro');
      ...
      registerBtn.onclick = async (e) => {
        ...
        console.log('auth-working.js: Botão de registro clicado');
        ...
        console.log('auth-working.js: Dados do formulário:', { name, email, cpf: cpf.substring(0, 3) + '***' });
        ...
        try {
          console.log('auth-working.js: === INICIANDO CADASTRO ===');
          
          // Primeiro, criar conta no Firebase Auth
          console.log('auth-working.js: 1. Criando conta no Firebase Auth...');
          const cred = await auth.createUserWithEmailAndPassword(email, pass);
          console.log('auth-working.js: ✅ Conta criada no Firebase Auth:', cred.user.uid);
          
          // Obter saldo inicial das configurações do sistema
          console.log('auth-working.js: 2. Obtendo saldo inicial...');
          ...
          try {
            ...
            if (settingsDoc.exists) {
              ...
              console.log('auth-working.js: ✅ Saldo inicial obtido:', initialBalance);
            } else {
              console.log('auth-working.js: ⚠️ Configurações do sistema não encontradas, usando saldo padrão');
            }
          } catch (error) {
            console.warn('auth-working.js: ⚠️ Erro ao obter configurações do sistema, usando saldo padrão:', error);
          }
          
          // Obter próximo ID sequencial
          console.log('auth-working.js: 3. Obtendo próximo ID sequencial...');
          ...
          try {
              ...
              if (counterDoc.exists) {
                  ...
                  console.log('auth-working.js: ✅ Próximo ID obtido:', nextUserId);
              } else {
                  console.log('auth-working.js: ⚠️ Contador não encontrado, criando inicial...');
              ...
              console.log('auth-working.js: ✅ Contador atualizado');
              
          } catch (error) {
              console.warn('auth-working.js: ⚠️ Erro ao obter contador de usuários, usando ID padrão:', error);
          }
          
          // Criar dados do usuário no Firestore
          console.log('auth-working.js: 4. Salvando dados no Firestore...');
          const userData = {
          ...
          console.log('auth-working.js: ✅ Dados salvos no Firestore:', userData);

          // Após cadastro bem-sucedido, fazer login automático
          console.log('auth-working.js: 5. Realizando login automático...');
          try {
            const loginCred = await auth.signInWithEmailAndPassword(email, pass);
            const loginToken = await loginCred.user.getIdToken();
            console.log('auth-working.js: ✅ Login automático realizado');
            
            // Buscar dados do usuário no Firestore
            console.log('auth-working.js: 6. Buscando dados do usuário no Firestore...');
            ...
            console.log('auth-working.js: ✅ Dados do usuário obtidos:', { loginUserId, loginFullName, loginBalance, loginRole });

            // Usar o AuthManager para salvar dados
            console.log('auth-working.js: 7. Salvando dados via AuthManager...');
            authManager.saveAuthData(loginToken, loginUserId, loginFullName, loginBalance, loginRole);
            console.log('auth-working.js: ✅ Dados salvos via AuthManager');
            ...
            setTimeout(() => {
              if (window.location.pathname.includes('index.html')) {
                ...
                if (typeof showLobby === 'function') {
                  console.log('auth-working.js: Chamando showLobby()...');
                  showLobby();
                } else {
                  console.log('auth-working.js: showLobby não está disponível, redirecionando...');
            ...
            console.error('auth-working.js: Erro no login automático:', loginError);
            showSuccess('Cadastro realizado com sucesso! Faça login.');
          }
        } catch(err) {
          console.error('auth-working.js: Erro no cadastro:', err);
          showError(err);
        }
      };
    }
    ...
    if (loginBtn) {
      console.log('auth-working.js: Configurando evento de login');
      loginBtn.onclick = async () => {
        console.log('auth-working.js: Botão de login clicado');
        ...
        try {
          console.log('auth-working.js: Realizando login...');
          console.log('auth-working.js: Dados do login:', { email, pass: pass.substring(0, 3) + '***' });
          
          // 1. Login no Firebase Auth
          console.log('auth-working.js: 1. Login no Firebase Auth...');
          const cred = await auth.signInWithEmailAndPassword(email, pass);
          console.log('auth-working.js: ✅ Login no Firebase Auth realizado:', cred.user.uid);
          
          // 2. Obter token
          console.log('auth-working.js: 2. Obtendo token...');
          const token = await cred.user.getIdToken();
          console.log('auth-working.js: ✅ Token obtido');
          
          // 3. Buscar dados no Firestore
          console.log('auth-working.js: 3. Buscando dados no Firestore...');
          const snap = await db.collection('users').doc(cred.user.uid).get();
          const data = snap.exists ? snap.data() : {};
          console.log('auth-working.js: ✅ Dados do Firestore:', data);
          ...
          console.log('auth-working.js: 4. Dados preparados:', { userId, fullName, balance, role });

          // 5. Usar o AuthManager para salvar dados
          console.log('auth-working.js: 5. Salvando dados via AuthManager...');
          authManager.saveAuthData(token, userId, fullName, balance, role);
          console.log('auth-working.js: ✅ Dados salvos via AuthManager');

          // 6. Verificar se o login foi bem-sucedido
          console.log('auth-working.js: 6. Verificando se está logado...');
          const isLoggedIn = authManager.isLoggedIn();
          console.log('auth-working.js: Usuário logado:', isLoggedIn);
          
          if (isLoggedIn) {
            const currentUser = authManager.getCurrentUser();
            console.log('auth-working.js: Dados do usuário atual:', currentUser);
          }

          console.log('auth-working.js: Login bem-sucedido:', { userId, fullName, balance });
          
          // 7. Redirecionar para o lobby
          console.log('auth-working.js: 7. Redirecionando para index.html...');
          window.location.href = 'index.html';
        } catch(err) {
          console.error('auth-working.js: Erro detalhado no login:', err);
          console.error('auth-working.js: Código do erro:', err.code);
          console.error('auth-working.js: Mensagem do erro:', err.message);
          
          // Tratamento específico para diferentes tipos de erro
          if (err.code === 'auth/network-request-failed') {
            console.error('auth-working.js: ERRO DE REDE DETECTADO!');
            if (authMessage) {
              authMessage.className = 'auth-message error';
              authMessage.textContent = 'Erro de conexão. Verifique sua internet e tente novamente.';
            }
          } else if (err.code === 'auth/user-not-found') {
            console.error('auth-working.js: USUÁRIO NÃO ENCONTRADO!');
            if (authMessage) {
              authMessage.className = 'auth-message error';
              authMessage.textContent = 'Usuário não encontrado. Verifique o email.';
            }
          } else if (err.code === 'auth/wrong-password') {
            console.error('auth-working.js: SENHA INCORRETA!');
            if (authMessage) {
              authMessage.className = 'auth-message error';
              authMessage.textContent = 'Senha incorreta. Tente novamente.';
            }
          } else if (err.code === 'auth/invalid-email') {
            console.error('auth-working.js: EMAIL INVÁLIDO!');
            if (authMessage) {
              authMessage.className = 'auth-message error';
              authMessage.textContent = 'Email inválido. Verifique o formato.';
            }
          } else if (err.code === 'auth/too-many-requests') {
            console.error('auth-working.js: MUITAS TENTATIVAS!');
            if (authMessage) {
              authMessage.className = 'auth-message error';
              authMessage.textContent = 'Muitas tentativas. Aguarde um pouco e tente novamente.';
            }
          } else if (err.code === 'auth/invalid-login-credentials') {
            console.error('auth-working.js: CREDENCIAIS INVÁLIDAS!');
            if (authMessage) {
              authMessage.className = 'auth-message error';
              authMessage.textContent = 'Email ou senha incorretos.';
            }
          } else {
            console.error('auth-working.js: ERRO DESCONHECIDO!');
            showError(err);
          }
        }
      };
    ...
    console.log('auth-working.js: Configuração concluída');
